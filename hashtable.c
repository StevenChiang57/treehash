...
void hash_undef(char *name){
  hash_node *prev = NULL, *cur;

  if((cur = hash_lookup(name)) != NULL){  // check if its in the hash table 
    for(cur = hashtab[hash(name)]; cur != NULL; cur = cur->next){
      if(strcmp(name, cur->name) == 0){
        if(prev == NULL)                    // first hash node case
