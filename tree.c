...
void sortarray(tree *t){ //shellsort but modified for list/trees
 int gap, i,j;
 tnode *temp;

 for(gap = t->size/2;gap > 0;gap /= 2)
    for(i = gap; i < t->size;i++)
...
void tree_printnodes_decreaseorder(tree* t, tnode* p){ 
  treestore(t,p);
  sortarray(t);
...
void tree_print_decreaseorder(tree* t) { tree_printnodes_decreaseorder(t, t->root);  printf("\n"); }
